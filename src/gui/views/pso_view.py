...
class PSOView(ctk.CTkFrame):
	def __init__(self, parent, app, **kwargs):
		...
		self.inputs_container = ctk.CTkFrame(body_frame, fg_color=theme.BG_CARD)
	...
	def _format_result(self, result) -> str:
		...
		if isinstance(sol, (list, tuple)):
			if len(sol) <= 20:
				lines.append(f"Best Particle Position  : {list(sol)}")
			else:
				lines.append(f"Best Particle Position  : {list(sol[:15])} ... (+ {len(sol) - 15} dims)")
		else:
